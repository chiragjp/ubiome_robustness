...
### brief interlude to get number of associations recovered as a function of if they came from studies in this study or not

litvib2 = read.csv('revisions/litvibs_2.csv')%>% filter(Cohort.used.in.sensitivity.analysis!='') %>% mutate(uid =  paste(full_feature_name,phenotype,sep='_')) %>% select(uid,Cohort.used.in.sensitivity.analysis) 

tempmerge = right_join(voefdr,litvib2) %>% mutate(`FDR significant at least once upon VoE` = if_else(is.na(`FDR significant at least once upon VoE`),0,1))

tempmerge %>% select(-uid) %>% table
...
ggplot(data=voe_all_lit_vibs_frac, aes(x=fraction_matching, group=phenotype, fill=phenotype)) +geom_histogram() +
...
voe_lit = read.csv('vibration_of_effects_output.csv')
...
voe_data_acvd_sig = voe_data %>% filter(p.value<fdr_sig) %>% mutate(significant=TRUE,num = nrow(.)) %>%  select(antibiotics_current_use,age,gender,triglycerides,ldl,cholesterol) %>% colSums %>% data.frame

voe_data_acvd_nosig = voe_data %>% filter(p.value>fdr_sig) %>% mutate(significant=FALSE,num = nrow(.)) %>% select(antibiotics_current_use,age,gender, triglycerides, ldl, cholesterol) %>% colSums %>% data.frame
