...
ggsave('~/Dropbox (HMS)/RagGroup Team Folder/Braden Tierney/vibration_of_effects_microbiome/repr/all_volcanos.pdf')
...
metaph_lit =  right_join(metaph,litvibs,by=c('phenotype','feature'='dependent_feature')) %>% filter(!is.na(adj))
...
for(p in phenotypes){
  ...
  #  x=ma_output$estimate
   # y=ma_output$adj
    p2=ggplot(data = voe_data, aes(x = estimate, y = -log10(p.value))) + geom_point(alpha=.5)  +theme(legend.position="bottom") +theme(legend.title = element_blank()) + xlab('Estimate')  + ggtitle(paste(p,f))+ theme(legend.text=element_text(size=6))+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05))+ xlim(-3,3) +ylim(0,3)# + geom_point(aes(x=x, y=-log10(y)),shape=23, fill="blue", color="darkred", size=3)#+ geom_hline(yintercept = -log10(fdr_sig),linetype='dashed')
