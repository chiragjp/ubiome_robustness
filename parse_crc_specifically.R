...
files = read.csv('crc_files',header=F) %>% unlist %>% unname
...
# 3 var
clr_3var_processed = map(clr_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_3var_processed) = clr_cols

logged_3var_processed = map(logged_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_3var_processed) = logged_cols

nonlogged_3var_processed = map(nonlogged_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(nonlogged_3var_processed) = nl_cols
...
clr_6var_processed = map(clr_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_6var_processed) = clr_cols

logged_6var_processed = map(logged_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_6var_processed) = logged_cols

nonlogged_6var_processed = map(nonlogged_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
...
clr_9var_processed = map(clr_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_9var_processed) = logged_cols

logged_9var_processed = map(logged_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_9var_processed) = clr_cols

nonlogged_9var_processed = map(nonlogged_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<0.05,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
...
clr_3var_processed = map(clr_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_3var_processed) = clr_cols

logged_3var_processed = map(logged_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_3var_processed) = logged_cols

nonlogged_3var_processed = map(nonlogged_3var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(nonlogged_3var_processed) = nl_cols
...
clr_6var_processed = map(clr_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_6var_processed) = clr_cols

logged_6var_processed = map(logged_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_6var_processed) = logged_cols

nonlogged_6var_processed = map(nonlogged_6var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(nonlogged_6var_processed) = nl_cols
...
clr_9var_processed = map(clr_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(clr_9var_processed) = clr_cols

logged_9var_processed = map(logged_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(logged_9var_processed) = logged_cols

nonlogged_9var_processed = map(nonlogged_9var, function(x) filter_unnest_feature_vib(x$vibration_output)  %>% mutate(pval_sig = if_else(p.value<bonf_equivalent,1,0)) %>% select(dependent_feature,pval_sig) %>%  group_by(dependent_feature) %>% summarize(pval_sig_once = sum(pval_sig)) %>% mutate(pval_sig_once = if_else(pval_sig_once == 0, 0, 1))) %>% reduce(inner_join,by='dependent_feature')
colnames(nonlogged_9var_processed) = nl_cols
